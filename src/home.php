<?php
/**
 * Home page template
 *
 * Display sticky posts in a full width layout on the home page.
 *
 * Based on [`cornerstone/page-templates/full-width.php`](https://github.com/thewirelessguy/cornerstone) and 
 * a tutorial on featured post layouts by [Chris Spooner](http://line25.com/tutorials/how-to-create-a-featured-post-layout-in-wordpress)
 *
 * @package WordPress
 * @subpackage wp-players-theme
 * @since wp-players-theme 1.0
 */

get_header(); ?>

<div class="row">
	<div id="primary" class="site-content small-12 medium-12 columns">
		<div id="content" role="main">
                    <?php query_posts(array("post__in"=>get_option('sticky_posts'))); ?>
			<?php if ( have_posts() ) : ?>

				<?php /* Start the Loop */ ?>
				<?php while ( have_posts() ) : the_post(); ?>
					<?php get_template_part( 'content', get_post_format() ); ?>
				<?php endwhile; ?>

			<?php else : ?>

				<article id="post-0" class="post no-results not-found">

				<?php if ( current_user_can( 'edit_posts' ) ) :
					// Show a different message to a logged-in user who can add posts.
				?>
					<header class="entry-header">
						<h1 class="entry-title"><?php _e( 'No posts to display', 'cornerstone' ); ?></h1>
					</header>

					<div class="entry-content">
						<p><?php printf( __( 'Ready to publish your first post? <a href="%s">Get started here</a>.', 'cornerstone' ), admin_url( 'post-new.php' ) ); ?></p>
					</div>

				<?php else :
					// Show the default message to everyone else.
				?>
					<header class="entry-header">
						<h1 class="entry-title"><?php _e( 'Nothing Found', 'cornerstone' ); ?></h1>
					</header>

					<div class="entry-content">
						<p><?php _e( 'Apologies, but no results were found. Perhaps searching will help find a related post.', 'cornerstone' ); ?></p>
						<?php get_search_form(); ?>
					</div>
				<?php endif; // end current_user_can() check ?>

				</article>

                        <?php endif; ?>
                    <?php wp_reset_query(); ?>

		</div>
	</div>
</div>

<?php get_footer(); ?>
